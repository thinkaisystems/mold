...
class AnomalyDetection:
    ...
    def save_nn_model(self,model,path):
    ...
    def export_nn_result(self,threshold,error_df,test_data,out_path):
        ...
        with open(file_name,'w') as out_file:
            out_file.write(x.to_json(orient='records'))
        ...
        with open(file_name,'w') as out_file:
            out_file.write(error_df_normal.to_json(orient='records'))
        
        file_name=out_path + "nn_abnormal.json"
        with open(file_name,'w') as out_file:
            out_file.write(error_df_abnormal.to_json(orient='records'))
    ...
    def export_svm_result(self,test,pred,out_path):
        ...
        with open(file_name,'w') as out_file:
            out_file.write(normal.to_json(orient='records'))
        
        file_name=out_path+"svm_abnormal.json"
        with open(file_name,'w') as out_file:
            out_file.write(abnormal.to_json(orient='records'))

        return normal,abnormal

    def load_nn_model(self, path,MODEL_SAVE_PATH):
